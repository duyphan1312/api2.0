...
const invokeTransaction = async (channelName, chaincodeName, fcn, args, username, org_name, transientData) => {
    try {
        ...
        let result
        let message;
        if (fcn === "createTreatment" || fcn === "createContact"||fcn === "createTreatment3"|| fcn === "createOutbreak" || fcn === "createPrivateTreatmentImplicitForOrg1"
            || fcn == "createPrivateTreatmentImplicitForOrg2") {
            result = await contract.submitTransaction(fcn, args[0], args[1], args[2], args[3], args[4],args[5],args[6]);
            message = `Successfully added the data with key ${args[0]}`

        } else if (fcn === "updateTreatmentStaff") {
            result = await contract.submitTransaction(fcn, args[0], args[1]);
            message = `Successfully changed Treatment owner with key ${args[0]}`
        } else if (fcn == "createPrivateTreatment" || fcn == "updatePrivateData") {
            console.log(`Transient data is : ${transientData}`)
            let TreatmentData = JSON.parse(transientData)
            console.log(`Treatment data is : ${JSON.stringify(TreatmentData)}`)
            let key = Object.keys(TreatmentData)[0]
            const transientDataBuffer = {}
            transientDataBuffer[key] = Buffer.from(JSON.stringify(TreatmentData.Treatment))
            ...
            return `Invocation require either createTreatment or updateTreatmentStaff as function but got ${fcn}`
        }
